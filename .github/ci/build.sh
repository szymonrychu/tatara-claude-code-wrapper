...
# the runtime DAG). Omitting --output builds the target without exporting an
# image: the daemon still solves the full test-guard DAG (every RUN executes, so
# the guard still runs) and keeps the layers in its own build cache for the
# runtime build that follows. We do NOT use `--output type=cacheonly` here: the
# shared buildkitd daemon does not register a `cacheonly` exporter and fails the
# solve with `exporter "cacheonly" could not be found`. No-output is the
# portable build-only invocation that predates that exporter.
