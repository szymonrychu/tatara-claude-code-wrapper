...
- REMOVED 2026-07-04 (branch feat/handoff-drop-s3, handoff-continuation design component 3) - S3 conversation persistence (issue #114) was never deployed and is now fully removed: internal/storage deleted, internal/convstore stripped to ProjectDirName/TranscriptDir only (still backs session.shouldResume's boot-crash fix). CHECKOUT_BRANCH read-only checkout for MR review is unaffected.
- REMOVED 2026-07-12 (task-centric wrapper redesign) - the chat-backed handoff preamble (CONVERSATION_OBJECT_KEY as handoff key, get_handoff/write_handoff first-submission preamble) and POST /v1/interject (mid-turn PTY injection) are both gone; there is no cross-pod resume any more, every pod's turn-0 is the same freshly rendered bundle. Replaced by AGENT_POD_TTL_SECONDS + a single operator-driven `handoff:true` turn admitted past the deadline (contract fix V6-6/V7-11) and GET /v1/session's contractVersion (contract G.10). The wrapper only enforces the TTL bound it is given - the OPERATOR owns the TTL clock (computes the deadline, decides when to send the handoff turn, tears the pod down).
...
- SHIPPED 2026-06-13 - tatara-deploy-harness baked skill: S1..S9 deploy state machine at templates/skills/tatara-deploy-harness/SKILL.md; skill-only (no wrapper Go/chart change); image rebuild ships via CI on merge.
...
- SHIPPED 2026-08-22 (claude-usage-gate-feed, semver:minor) - new `cmd/cc-statusline` binary is the live data feed for tatara-operator's Claude subscription usage gate: installed via `settings.json`'s `statusLine` key, it forwards claude's `rate_limits` stdin payload to the wrapper over a new loopback route (`POST /internal/account-usage`, `internal/ratelimit.Latest` newest-wins holder), and the turn-complete callback carries the newest snapshot to the operator as `accountUsage`, omitted entirely when none has been observed. See MEMORY.md for the `resets_at` unix-vs-RFC3339 divergence, the print-mode/PTY dependency, and the one-leg-unwitnessed honest residual. Companion changes land in tatara-operator (Task status ingest + fleet-wide leader-only reconciler) and tatara-helmfile (per-window thresholds).
