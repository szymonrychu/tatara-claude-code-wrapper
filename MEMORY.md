...
- 2026-06-20 (operator#74, tatara/task-scan-qe-fbb9g): per-Project agent customization - wrapper side (subtask 3 of 5). The operator mounts customization at /etc/wrapper and the wrapper consumes it; 4 of the 6 asks needed NO wrapper change (prompt -> existing PROJECT_CLAUDE_MD_PATH, mcp -> MCP_OVERLAY_DIR mcp.d merge, skills -> SKILLS_SRC_DIRS, model/effort/permission already wired). Added the 2 that were missing: (1) PLUGINS - new internal/bootstrap/plugins.go: `pluginConfig([]PluginSpec)` renders the operator-mounted plugins.json (PLUGINS_PATH, default /etc/wrapper/plugins.json) into the DECLARATIVE settings.json keys `extraKnownMarketplaces` + `enabledPlugins`. KEY DECISION: chose declarative settings over the interactive `/plugin install` / `/plugin marketplace add` CLI because those have NO non-interactive flag and the agent runs headless (no TTY) - declarative plugins load at boot without a prompt (per code.claude.com/docs discover-plugins + settings). Plugin contract: PluginSpec.Name is the full `plugin@marketplace` id (-> enabledPlugins key verbatim); Source (github "owner/repo" or git URL) registers that marketplace under the name taken from Name's `@suffix`, so the extraKnownMarketplaces key always matches the enabledPlugins reference. Source classified git-URL vs github by "://"/"git@" prefix. Empty Source = enable by name only (marketplace already known). (2) SETTINGS PASSTHROUGH - writeSettings now merges the operator-mounted settings-extra.json (EXTRA_SETTINGS_PATH, default /etc/wrapper/settings-extra.json) FIRST (lowest priority) so non-managed knobs like maxParallelism pass through, then ALWAYS overwrites the operator-managed keys (hooks, enableAllProjectMcpServers, effortLevel, permissions, plugin keys) so extra settings can never clobber effort/permissions. Invalid extra JSON is a hard bootstrap error; a missing file is fine (readBytesOrDefault nil). plugins.json parse is best-effort (logged, ignored on malformed). config.go wires both paths with /etc/wrapper defaults matching the operator mounts. No operator/helmfile change here; rollout = helmfile chart bump + image tag (subtask 5).
