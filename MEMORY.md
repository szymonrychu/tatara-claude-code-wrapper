...
- 2026-06-14 (B2): Added `internal/metricspush` push client: ships the wrapper's gathered prometheus families to the operator push-receiver (B1) on a Pushgateway-style grouping path `<METRICS_PUSH_URL>/metrics/job/<job>/run_id/<run_id>/pod/<pod>` (PUT to push, DELETE to drop). Periodic push (METRICS_PUSH_INTERVAL_SECONDS, default 15) keeps a short-lived pod observable; `Shutdown` does a final push then DELETEs the run's series (operator TTL is the hard-kill backstop). All best-effort: failures log WARN, never block exit, counted in `ccw_metrics_push_total{op,result}`. No auth header - mirrors the in-cluster turn-complete callback channel (operator internal endpoint takes no bearer token). run_id is generated per run (env `RUN_ID` override); pod from downward-API `POD_NAME` (deployment.yaml fieldRef) falling back to hostname. Config via the camelCase->UPPER_SNAKE configmap convention (metricsPushUrl/metricsPushIntervalSeconds/metricsJob). Operator-built pods (operator pod.go) don't yet inject METRICS_PUSH_URL/POD_NAME - that is the B1/operator-side wiring.
